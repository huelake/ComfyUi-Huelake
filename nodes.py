...
class XYGridAccumulator(FeedbackNode):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "images": ("IMAGE",),
                "xy_grid_control": ("XY_GRID_CONTROL",),
                "index": ("INT", {"default": 0, "min": 0}),  # New index input
            },
            "hidden": {
                "unique_id": "UNIQUE_ID",
            }
    ...
    # Class variable to store accumulated images per unique_id
    image_batches = {}

    def run(self, images, xy_grid_control, index, unique_id):
        count, row_texts, column_texts, max_columns, font_size, grid_gap = xy_grid_control
        
        # Initialize batch for this unique_id if it doesn't exist
        if unique_id not in self.image_batches:
            self.image_batches[unique_id] = torch.Tensor()
        
        # Reset batch if index is 0 (start of new sequence)
        if index == 0:
            self.image_batches[unique_id] = torch.Tensor()
        
        # Accumulate images
        if self.image_batches[unique_id].numel() == 0:
            self.image_batches[unique_id] = images
        else:
            self.image_batches[unique_id] = torch.cat((self.image_batches[unique_id], images))
        
        # Get the images we need for this grid
        total_images = self.image_batches[unique_id]
        processed_images = total_images[:count]
...
class XYGridHelper():
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "row_list": ("LIST",),
                "column_list": ("LIST",),
                "index": ("INT", {"default": 0, "min": 0}),  # Changed from optional to required
            },
            "optional": {
                "row_prefix": ("STRING", {"default": ""}),
                "column_prefix": ("STRING", {"default": ""}),
    ...
    RETURN_TYPES = ("AXIS_VALUE", "AXIS_VALUE", "XY_GRID_CONTROL", "INT")
    RETURN_NAMES = ("row_value", "column_value", "xy_grid_control", "next_index")
    FUNCTION = "run"
    CATEGORY = "QQNodes/XYGrid"
    ...
    def run(self, row_list, column_list, index, row_prefix="", column_prefix="", page_size=10, label_length=50, font_size=50, grid_gap=20):
        total_grid_images = len(row_list) * len(column_list)
        adjusted_index = index % total_grid_images
        ...
        # Remove reset parameter from xy_grid_control tuple
        xy_grid_control = (min(images_pr_page, total_grid_images - page_index * page_size), 
                          row_annotation, column_annotation, len(column_list), font_size, grid_gap)
        
        # Calculate next index for convenience
        next_index = (index + 1) % total_grid_images
        ...
        return {"result": (
            ...
            xy_grid_control,
            next_index,
        ), "ui": {"total_images": [total_grid_images], "current_index": [index]}}
